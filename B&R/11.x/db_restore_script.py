...
def cleanup_mongodb(namespace):
    ...
    try:
        dbs_output = subprocess.check_output(
            f'mongosh "mongodb://{mongo_user}:{mongo_passwd}@localhost:{os.environ['LOCAL_MONGO_PORT']}/?directConnection=true&serverSelectionTimeoutMS=20000&authSource=admin" --eval "JSON.stringify(db.getMongo().getDBs().databases)"',
            shell=True
        ).decode().strip()
